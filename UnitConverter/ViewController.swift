...
class ViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        HttpService.getJSON("\(FrameworkConstants.currencyAPIBaseURLPrefix)\(baseCurrency.description)", callback: { (jsonData) -> Void in
            self.converter.updateRatesWithJSONData(baseCurrency, jsonData: jsonData)
            print(self.converter.getTargetCurrencyFormattedString(100, sourceCurrency: Currency.EUR, targetCurrency: Currency.CNY))
        })
        ...
        for country in filteredCountries {
            print("\(country.currencyName) \(country.currency.symbol()) \(country.flagCode)")
        }
    }
