...
public class Country{
    ...
    public init(name: String, code: String,currency: Currency = .USD, currencyName: String, isFlagCodeAvailable: Bool){
        ...
        self.flagCode = "\(FlagCode(char: Array(code.characters)[0]).rawValue)\(FlagCode(char: Array(code.characters)[1]).rawValue)"
...
public class Countries{
    ...
    public func getCountriesWithAvailableExchangeRate() -> [Country] {
        ...
        return valueArray.sort { $0.name < $1.name }
...
//        return sorted(valueArray){ $0.name < $1.name}
    ...
    public init(){
        ...
        do {
            let csvData = try CSV(contentsOfURL: csvURL, delimiter: tab, encoding: NSUTF8StringEncoding)
            if error != nil {
